...
class InterestAccountsController extends Controller
{
    /**
     * 參數設定
     *
    ...
    protected $page_title = '紅利點數支出';
    protected $menu_title = '紅利點數支出';
    protected $route_code = 'interest_account';
    protected $view_data = [];
    ...
    public function summary($start = null, $end = null, $period_type = 'd')
    {
    	...
    	$result = $this->statisticService->getStatByRange($start,$end,'receive_center_4',$period_type); 
        $data =[
            'datas' => $result['datas'],
    ...
    public function members($start, $end)
    {
        ...
        $result = $this->statisticService->getMemberStatByRange($start,$end,'receive_center_4'); 
    ...
    public function detail($user_id, $start, $end)
    {
        ...
        $account = $accounts->where('type','4')->first();
