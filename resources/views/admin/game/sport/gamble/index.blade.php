...
<div class="box-header with-border">
    ...
    <!-- 上一頁 -->
    <div class="text-left">
        <a href="{{ route('admin.game.'.$route_code.'.index',$category->id) }}"><button class="btn btn-default btn-sm" data-toggle="tooltip" title="回上一頁"><i class="fa fa-fw fa-reply"></i></button></a>
    </div>
...
<div class="box-body" >
    <h4>賽程資訊</h4>
    <p>比賽類型：{{ $category->name }}</p>
    <p>比賽時間[當地]：{{ $sport->start_datetime }}</p>
    <p>比賽時間[台灣]：{{ $sport->taiwan_datetime }}</p>
    <p>賽程狀態：{!! config('game.sport.status.'.$sport->status) !!}</p>
    <hr>
    <h4>比分資訊</h4>
    <div class="row">
        <div class="col-sm-6">
            <h5>主隊</h5>
            <ul class="list-group">  
                <li class="list-group-item">{{ $home_team->name }}：{{ $home_team->score }}</li>
            </ul>
        </div>
        <div class="col-sm-6">
            <h5>客隊</h5>
            <ul class="list-group">  
                <li class="list-group-item">{{ $away_team->name }}：{{ $away_team->score }}</li>
            </ul>
        </div>
    </div>
     <hr>
    <h4>賭盤資訊</h4>
    <p style="color:red;">無人下過注，賭盤狀態為「未開放下注」才能刪除</p>
