...
<div class="box-body" >
    <p style="color:green;">確認：表示已經匯款給會員</p>
    <p style="color:red;">拒絕：拒絕會員的申請，退還給會員</p>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>{!! config('shop.withdrawal.confirm_status.'.$data->confirm_status) !!}
                        ...
                        <br><p style="font-size:10px">時間：{{ $data->confirm_at }}({{ $data->confirm_admin->name }})</p>
                        @endif
                    ...
                    <td>
                        ...
                        <button type="button" class="btn btn-success btn-sm confirm" data-id="{{ $data->id }}" data-name="{{ $data->member->name }}" data-amount="{{ $data->amount }}" data-status="1">確認</button>
                        <button type="button" class="btn btn-danger btn-sm confirm" data-id="{{ $data->id }}" data-name="{{ $data->member->name }}" data-amount="{{ $data->amount }}" data-status="2">拒絕</button>
                        @endif
                    </td>
                </tr>
                @endforeach
...
<script>
    $(document).ready(function() {
        ...
        table.on("click", ".confirm", function(e) {
            ...
            //確認訊息
            $.confirm({
                text: name+"(#"+id+" 金額:"+amount+" ) 申請紀錄處理方式為「 "+status_name+" 」 ?",
