...
function getSuccessAccountRecords(records,game_info=''){
    var content = game_info+'<table class="table" style="width:90%; margin-left:5%; text-align:center;"><thead><tr><th style="text-align:center; vertical-align:middle;">幣別</th><th style="text-align:center; vertical-align:middle;">時間/單號</th><th style="text-align:center; vertical-align:middle;">金額</th></tr></thead>';  
    for (var key in records) {
        content += '<tr><td rowspan="2" style="vertical-align:middle;"><div class="'+records[key].account_name+'"></div></td><td style="vertical-align:middle;">'+records[key].created_at+'</td><td rowspan="2" style="vertical-align:middle;">'+records[key].amount+'</td></tr><tr><td style="vertical-align:middle;">'+records[key].bet_number+'</td></tr>';
    };  
    content += '</table>'
    return content;    
}
...
function getBetAmount(t_cash,t_run,t_interest,t_right,account_data){
    ...
    content += '<p>'+account_data[4]['icon']+'<b>'+account_data[4]['name']+'：</b>'+numeral(interest).format('0,0')+'（餘額:'+numeral(account_data[3]['amount']).format('0,0')+'）</p>';
    content += '<p>'+account_data[3]['icon']+'<b>'+account_data[3]['name']+'：</b>'+numeral(right).format('0,0')+'（餘額:'+numeral(account_data[4]['amount']).format('0,0')+'）</p>';
    return content;
